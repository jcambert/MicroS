using System;
using System.Collections.Generic;
<% if(props.filter(property=>property.dynamic).some(e=>true)){%>using WeCommon;<%}%>
<%- props.filter(prop=>!prop.isprimitive).map(prop=>`using ${appname}.domain.${prop.name}s.Domain;`).join('\n') %>
/// <summary>
/// This file was generated by the yeoman generator "generator-micros"
/// @author: <%=author.name%>
/// @email: <%=author.email%>
/// @created_on: <%= new Date()%>
/// </summary>
namespace <%=appname%>.domain.<%= domain%>s.Messages.Events
{
    public class <%= pascalDomain%>Updated : <%= pascalDomain%>Created
    {
        public <%= pascalDomain%>Updated(Guid id,<%- props.filter(prop=>prop.isprimitive).map(prop=>prop.dynamic?`Property<${prop.type}> ${changeCase.lowerCase(prop.name)}`: `${prop.type} ${changeCase.lowerCase(prop.name)}`).concat(props.filter(prop=>!prop.isprimitive).map(prop=>`List<${prop.name}> ${changeCase.lowerCase(prop.name)}s`)).join(',') %>) 
        : base(id,<%- props.filter(prop=>prop.isprimitive).map(prop=>`${changeCase.lowerCase(prop.name)}`).concat(props.filter(prop=>!prop.isprimitive).map(prop=>`${changeCase.lowerCase(prop.name)}s`)).join(',') %>)
        {
        }
    }
}
